...
public class UserController {
    ...
    @GetMapping("/")
    public ResponseEntity<BaseApiResponse<UserResponse>> getUser(
            @Validated @RequestParam(name = "ext_id") UUID extId
    ) {
        return ResponseEntity.ok(BaseApiResponse.ok(userService.findUserByExtIdOrThrow(extId)));
    }

    @GetMapping("/me")
    public ResponseEntity<BaseApiResponse<UserResponse>> getMe(
            @AuthenticationPrincipal UUID extId
    ) {
        return ResponseEntity.ok(
                BaseApiResponse.ok(userService.findUserByExtIdOrThrow(extId))
        );
    }

    @PostMapping("/register")
    public ResponseEntity<BaseApiResponse<UUID>> registerUser(
